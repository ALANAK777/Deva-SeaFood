  ...
  const updateOrderStatus = async (newStatus: OrderStatus) => {
    ...
    try {
      ...
      if (newStatus === 'out_for_delivery' && verificationCode) {
        ...
        const orderDisplay = selectedOrder.order_number || `#${selectedOrder.id.substring(0, 8)}`;
        showToast(`✅ Order ${orderDisplay} has been successfully delivered to the customer.`, 'success');
      } else {
        const orderDisplay = selectedOrder.order_number || `#${selectedOrder.id.substring(0, 8)}`;
        showToast(`Order ${orderDisplay} status updated to ${getStatusDisplayName(newStatus)}`, 'success');
      }
    } catch (error: any) {
